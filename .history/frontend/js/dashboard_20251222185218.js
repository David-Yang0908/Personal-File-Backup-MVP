    ...
    try {
        ...
        if (fileKey.startsWith('http')) {
            ...
            if (response.ok) {
                ...
                // 如果 fetch 被擋下 (CORS)，則使用最後手段：直接跳轉下載
                window.location.href = fileKey;
                return;
            }
        }

        // --- 情況 B：S3 檔案 (My Collection) ---
        console.log(`正在請求 S3 下載連結: ${fileKey}`);
        ...
        if (data.downloadUrl) {
            ...
            // 重要：這裡不需要 a.download = ... 因為檔名已由 Lambda 指定
            document.body.appendChild(a);
            a.click(); // 觸發下載
            document.body.removeChild(a);
        }
