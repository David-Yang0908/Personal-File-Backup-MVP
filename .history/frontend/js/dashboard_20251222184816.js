...
async function handleDownloadFile(fileKey) {
    try {
        ...
        if (fileKey.startsWith('http')) {
            console.log("正在請求外部資源直接下載:", fileKey);
            showToast('正在下載外部檔案...', 'info');

            // 使用 fetch 抓取檔案內容並轉為 Blob
            const response = await fetch(fileKey);
            
            // 如果發生 CORS 錯誤 (部分學術網站會擋)，則退而求其次使用隱藏下載
            if (!response.ok) {
            ...
            a.href = url;
            a.download = fileKey.split('/').pop() || 'document.pdf';
        ...
        // --- 情況 B：自己的 S3 檔案 (My Collection) ---
        // 呼叫你的 RequestDownloadHandler API
        const apiUrl = `${AWS_CONFIG.apiGatewayUrl}/download?fileName=${encodeURIComponent(fileKey)}`;
        ...
        if (data.downloadUrl) {
            ...
            // 這裡不需要 window.open，因為 Lambda 已經加了 attachment 標頭
            a.click();
        }
    } catch (error) {
        console.error("下載失敗:", error);
        // 如果 fetch 被 CORS 擋住，最後的保險手段是單純的連結跳轉
        if (fileKey.startsWith('http')) {
