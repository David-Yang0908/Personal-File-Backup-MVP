...
async function handleDownloadFile(fileKey) {
    try {
        showToast('正在請求下載連結...', 'info');

        // 1. 呼叫你的 API Gateway (RequestDownloadHandler)
        // 假設你的 API 路徑是 /download?fileName=...
        const response = await fetch(`${AWS_CONFIG.apiGatewayUrl}/download?fileName=${encodeURIComponent(fileKey)}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${(await getCurrentSession()).getIdToken().getJwtToken()}`
            }
        });
