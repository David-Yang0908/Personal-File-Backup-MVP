...
async function handleDownloadFile(fileKey) {
    try {
        console.log(`正在請求下載連結: ${fileKey}`);
        
        // 1. 確保 API 網址正確
        // 注意：請確認 AWS_CONFIG.apiGatewayUrl 結尾沒有斜線
        const apiUrl = `${AWS_CONFIG.apiGatewayUrl}/download?fileName=${encodeURIComponent(fileKey)}`;
        
        console.log("呼叫 API 網址:", apiUrl);
        ...
        if (data.downloadUrl) {
            ...
            throw new Error("API 回傳結果中找不到 downloadUrl");
        }
