...
async function handleDownloadFile(fileKey) {
    try {
        // 判斷是否為外部網址 (例如 arXiv 的論文)
        if (fileKey.startsWith('http')) {
            console.log("偵測到外部資源：開啟預覽並嘗試下載", fileKey);
            
            // 動作 1：在新分頁開啟預覽
            window.open(fileKey, '_blank');

            // 動作 2：嘗試觸發瀏覽器下載
            // 注意：對於跨來源 (Cross-Origin) 網址，瀏覽器安全機制可能會攔截 download 屬性，
            // 若該伺服器未設定強制下載標頭，瀏覽器仍會傾向於直接開啟。
